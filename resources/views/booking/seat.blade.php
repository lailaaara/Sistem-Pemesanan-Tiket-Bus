                        ...
                        <div class="seat-row">
                            @foreach(['A', 'B'] as $col)
                                ...
                                <button class="seat {{ in_array($seatNo, $takenIds) ? 'taken' : 'available' }}"
                                    data-seat="{{ $seatNo }}" {{ in_array($seatNo, $takenIds) ? 'disabled' : '' }}>
                                    {{ $seatNo }}
                                </button>
                            @endforeach
            ...
            <div class="booking-summary">
                <div class="summary-card">
                    ...
                    <div class="journey-summary">
                        <div class="js-item">
                            ...
                            <div>
                                ...
                                <div class="js-time">
                                    {{ $jadwal ? \Carbon\Carbon::parse($jadwal->jam_berangkat)->format('l, d M Y • H:i') : '—' }}
                                </div>
                            </div>
                        </div>
                        <div class="js-item">
                    ...
                    <div class="summary-meta">
                        <div class="meta-row"><span>Bus
                                Operator</span><span><strong>{{ ($jadwal->nama_bus ?? 'BusMania') . ' • ' . ($jadwal->kelas ?? 'Eksekutif') }}</strong></span>
                        </div>
                        <div class="meta-row"><span>Nomor
                                Bus</span><span><strong>{{ $jadwal->no_polisi ?? 'LB-2045-A' }}</strong></span></div>
                    </div>
                </div>

                <div class="summary-card">
                    <h4>DETAIL KURSI & HARGA</h4>
                    <div class="meta-row"><span>Kursi Terpilih</span><span id="selected-seat-display"
                            class="seat-badge">—</span></div>
                    <div class="meta-row"><span>Harga Tiket (1x)</span><span id="price-display">Rp
                            {{ $jadwal ? number_format($jadwal->harga, 0, ',', '.') : '0' }}</span></div>
                    <div class="meta-row"><span>Biaya Layanan</span><span>Rp 5.000</span></div>
                    <div class="meta-row total"><span>Total Pembayaran</span><span id="total-display">Rp
                            {{ $jadwal ? number_format($jadwal->harga + 5000, 0, ',', '.') : '5.000' }}</span></div>

                    <a href="{{ route('booking.checkout', ['jadwal_id' => $jadwal->id_jadwal ?? '', 'kursi' => '']) }}"
                        id="btn-lanjut" class="btn btn-primary btn-block mt-4 disabled-btn">
                        Lanjut ke Pembayaran
                    </a>
                    <p class="summary-note">Dengan melanjutkan, Anda menyetujui <a href="#">Syarat & Ketentuan</a> yang
                        berlaku.</p>
                </div>
            </div>
        </div>
